...
import { X, ArrowLeft, ZoomIn, Calendar, MapPin } from 'lucide-react';
...
const galleryImages = [
  ...
  },
  {
    ...
    alt: 'Award ceremony at hackathon',
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="fixed top-0 left-0 right-0 z-50 border-b border-foreground/10 bg-background/90 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-6 lg:px-12 py-4 flex items-center justify-between">
          ...
          <span className="font-mono text-xs text-foreground/40 tracking-widest uppercase">QNexus India / Gallery</span>
        </div>
      </div>
      ...
      <div className={`pt-32 pb-16 px-6 max-w-7xl mx-auto transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
        ...
        <p className="text-lg text-foreground/55 max-w-xl">
          Highlights from QNexus India events, workshops, hackathons, and community gatherings across the country.
        </p>
        <div className="flex items-center gap-6 mt-6 text-sm font-mono text-foreground/40">
          <span>{galleryImages.length}+ photos</span>
          <span>·</span>
          <span>2024–2026</span>
          <span>·</span>
          <span>Across India & Online</span>
        </div>
      </div>
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4">
                ...
                <div className="flex items-center gap-3 mt-1">
                  <span className="text-white/70 text-xs flex items-center gap-1"><Calendar className="w-3 h-3" />{img.date}</span>
                  <span className="text-white/70 text-xs flex items-center gap-1"><MapPin className="w-3 h-3" />{img.location}</span>
                </div>
              </div>
              {img.featured && (
                <div className="absolute top-3 left-3 px-2.5 py-1 bg-amber-500 text-black text-[10px] font-mono font-bold rounded-full">
                  FEATURED
                </div>
              )}
            </div>
